...
moveprompt:     .asciiz "\nEnter your move ('a' = left, 'd' = right, 's' = drop, 'r' = rotate 'q' = quit): "
turnprompt: .asciiz "Moves: "
newline:    .asciiz "\n"
hash:      .asciiz "#"
empty:      .asciiz "."
row_end:    .asciiz "\n"
...
#Debug Messages
.align 2
align_error_msg: .asciiz "Alignment error occurred!\n"
...
block_cell4:
	...
	add $s7, $t2, $zero   #Stores the row of the occupied cell in $s1
	add $t3, $t3, 1	#Else, add to column index
	add $t0, $t0, 4	#Move right to next index
	...
	li $t3, 0 #block drop distance (put here so it doesn't reset)
	j start_drop_check
...
update_cell1:
	...
	li $t6, 0
	sw $t6, 0($t0)    #Loads value of cell pointer into $t6
...
update_cell3:
	...
	add $t0, $t7, $t0 #Now $t0 is a pointer to cell
	li $t6, 0
	sw $t6, 0($t0)    #Loads value of cel1 pointer into $t6
